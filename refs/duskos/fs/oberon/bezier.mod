MODULE Bezier;
  ...
  BEGIN DEC(x, 3); DEC(y, 3);
    IF (x >= f.X) & (x+7 < f.X1) & (y >= f.Y) & (y+7 < f.Y1) THEN
      IF col = Display.black THEN Display.FillBlock(Display.black, x, y, 7, 7)
      ELSE Display.PaintPattern(col, GraphicFrames.tack, x, y)
      END
    END
  END mark;
  ...
  BEGIN
    ...
    IF b.cubic THEN
      bezCubic(f, b.x + mx, yS + my, b.x + b.dx1 + mx, b.y + b.dy1 + my, b.x + b.dx2 + mx, b.y + b.dy2 + my, b.x + b.w + mx, yE + my, col);
    ELSE
      bezQuad(f, b.x + mx, yS + my, b.x + b.dx1 + mx, b.y + b.dy1 + my, b.x + b.w + mx, yE + my, col);
    END;
  END bezier;
  ...
  BEGIN dst.x := src.x; dst.y := src.y; dst.w := src.w; dst.h := src.h; dst.col := src.col;
    ...
    dst(Bezier).dx1 := src(Bezier).dx1; dst(Bezier).dy1 := src(Bezier).dy1;
    dst(Bezier).dx2 := src(Bezier).dx2; dst(Bezier).dy2 := src(Bezier).dy2;
  ...
  BEGIN
    ...
    RETURN (obj.x <= x) & (x < obj.x + w) & (ABS(y-y0)*w - (x-obj.x)*ABS(h) < w*4)
  END Selectable;
  ...
  BEGIN Files.ReadByte(R, len); obj(Bezier).swap := len # 0;
    ...
    Files.ReadInt(R, xy); obj(Bezier).dy1 := xy DIV 10000H; obj(Bezier).dx1 := xy * 10000H DIV 10000H;
    IF obj(Bezier).cubic THEN
      Files.ReadInt(R, xy); obj(Bezier).dy2 := xy DIV 10000H; obj(Bezier).dx2 := xy * 10000H DIV 10000H;
    END;
  ...
  BEGIN Graphics.WriteObj(W, cno, obj);
    ...
    Files.WriteInt(W, obj(Bezier).dx1 +  obj(Bezier).dy1 * 10000H);
    IF (obj(Bezier).cubic) THEN Files.WriteInt(W, obj(Bezier).dx2 +  obj(Bezier).dy2 * 10000H) END;
  ...
  BEGIN G := GraphicFrames.Focus();
    IF (G # NIL) & (G.mark.next # NIL) & (G.mark.next.next # NIL) THEN
      ...
      NEW(b); b.col := GraphicFrames.OberonCurCol;
      b.dx1 := G.mark.next.x - G.x; b.dy1 := G.mark.next.y - G.y;
      b.cubic := G.mark.next.next.next # NIL;
      IF (b.cubic) THEN
        b.dx2 := xE - G.x; b.dy2 := yE - G.y;
      ...
      ELSE b.x := xE; b.swap := yE >= yS;
        ...
        IF b.cubic THEN xS := b.dx1; b.dx1 := b.dx2; b.dx2 := xS; xS := b.dy1; b.dy1 := b.dy2; b.dy2 := xS END;
      END;
      DEC(b.x, G.x); DEC(b.y, G.y); b.do := method;
      DEC(b.dx1, b.x); DEC(b.dx2, b.x); DEC(b.dy1, b.y); DEC(b.dy2, b.y);
