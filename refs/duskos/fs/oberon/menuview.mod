(* Source: https://github.com/Spirit-of-Oberon/wirth-personal
   Filename: people.inf.ethz.ch/wirth/ProjectOberon/Sources/MenuViewers.mod.txt
...
TYPE Viewer* = POINTER TO ViewerDesc;
  ...
  ModifyMsg* = RECORD (Viewers.FrameMsg)
    id: INTEGER;
    dY, Y, H: INTEGER
  END;

PROCEDURE Copy(V: Viewer; VAR V1: Viewer);
VAR Menu, Main: Viewers.Frame; M: Oberon.CopyMsg;
BEGIN
...
BEGIN
  Display.FillBlock(Oberon.FrameColor, V.X, V.Y, 1, V.H);
  Display.FillBlock(Oberon.FrameColor, V.X + V.W - 1, V.Y, 1, V.H);
  Display.FillBlock(Oberon.FrameColor, V.X + 1, V.Y, V.W - 2, 1);
  Display.FillBlock(Oberon.FrameColor, V.X + 1, V.Y + V.H - 1, V.W - 2, 1)
END Draw;
...
BEGIN dH := V.Y - newY;
  IF dH > 0 THEN
    Display.FillBlock(Display.black, V.X + 1, newY + 1, V.W - 2, dH);
    Display.FillBlock(Oberon.FrameColor, V.X, newY, 1, dH);
    Display.FillBlock(Oberon.FrameColor, V.X + V.W - 1, newY, 1, dH);
    Display.FillBlock(Oberon.FrameColor, V.X + 1, newY, V.W - 2, 1)
  END
END Extend;

PROCEDURE Reduce(V: Viewer; newY: INTEGER);
BEGIN Display.FillBlock(Oberon.FrameColor, V.X + 1, newY, V.W - 2, 1)
END Reduce;
...
BEGIN dH := V.H - oldH;
  IF dH > 0 THEN
    Display.FillBlock(Oberon.FrameColor, V.X, V.Y + oldH, 1, dH);
    Display.FillBlock(Oberon.FrameColor, V.X + V.W - 1, V.Y + oldH, 1, dH);
    Display.FillBlock(Oberon.FrameColor, V.X + 1, V.Y + V.H - 1, V.W - 2, 1)
  END
END Grow;

PROCEDURE Shrink(V: Viewer; newH: INTEGER);
BEGIN Display.FillBlock(Oberon.FrameColor, V.X + 1, V.Y + newH - 1, V.W - 2, 1)
END Shrink;

PROCEDURE Adjust(F: Viewers.Frame; id, dY, Y, H: INTEGER);
VAR M: ModifyMsg;
BEGIN
  M.id := id; M.dY := dY; M.Y := Y; M.H := H;
  F.handle(F, M);
  F.Y := Y; F.H := H
END Adjust;
...
BEGIN
  ...
  Draw(V);
  Menu.X := V.X + 1; Menu.Y := V.Y + V.H - 1;
  Menu.W := V.W - 2; Menu.H := 0;
  Main.X := V.X + 1; Main.Y := V.Y + V.H - V.menuH;
  Main.W := V.W - 2; Main.H := 0;
...
BEGIN (*Keys # {}*)
  ...
  Display.InvertBlock(
    Display.white, V.X + 1, V.Y + V.H - 1 - V.dsc.H, V.W - 2, V.dsc.H);
...
BEGIN
  ...
  Oberon.MouseMsg:
    ...
    END |
  Oberon.KeyMsg: Menu.handle(Menu, M); Main.handle(Main, M) |
  Oberon.ControlMsg:
    IF M.id = Oberon.mark THEN
      X := M.X; Y := M.Y;
      Oberon.DrawMouseArrow(X, Y); Oberon.DrawPointer(X, Y)
