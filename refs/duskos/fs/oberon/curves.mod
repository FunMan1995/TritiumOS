MODULE Curves;  (*NW 8.11.90 / 18.4.2013*)
    ...
    CASE M OF  GraphicFrames.DrawMsg:
      ...
      IF (x < f.X1) & (f.X <= x+w) & (y < f.Y1) & (f.Y <= y+h) THEN
        IF obj.kind = 0 THEN  (*up-line*)
          ...
          END
        ELSIF obj.kind = 1 THEN  (*down-line*)
          IF M.mode = 0 THEN
  ...
  BEGIN
    CASE obj OF Curve:
    IF obj.kind <= 1 THEN  (*line*)
      w := obj.w; h := obj.h;
      IF obj.kind = 1 THEN y0 := obj.y + h; h := -h ELSE y0 := obj.y END ;
      res := (obj.x <= x) & (x < obj.x + w) & (ABS(y-y0)*w - (x-obj.x)*ABS(h) < w*4)
  ...
  BEGIN
    CASE M OF Graphics.ColorMsg: obj.col := M.col END
  END Change;
  ...
  BEGIN
    CASE obj OF Curve:
    Files.ReadByte(R, len); Files.ReadByte(R, len); obj.kind := len;
    Files.ReadByte(R, len); obj.lw := len
    END
  END Read;
  ...
  BEGIN
    CASE obj OF Curve:
      ...
      Files.WriteByte(W, 2); Files.WriteByte(W, obj.kind); Files.WriteByte(W, obj.lw)
    END
  END Write;
  ...
  BEGIN G := GraphicFrames.Focus();
    IF (G # NIL) & (G.mark.next # NIL) THEN
      ...
      NEW(c); c.col := GraphicFrames.OberonCurCol;
  ...
  BEGIN G := GraphicFrames.Focus();
    IF (G # NIL) & (G.mark.next # NIL) THEN
      ...
      IF r > 4 THEN
        ...
        c.kind := 2; c.col := GraphicFrames.OberonCurCol;
  ...
  BEGIN G := GraphicFrames.Focus();
    IF (G # NIL) & (G.mark.next # NIL) & (G.mark.next.next # NIL) THEN
      ...
      IF (a > 4) & (b > 4) THEN
        ...
        c.kind := 3; c.col := GraphicFrames.OberonCurCol;
        c.lw := Graphics.width; c.do := method;
        Graphics.Add(G.graph, c);
        GraphicFrames.Defocus(G); GraphicFrames.DrawObj(G, c)
      END
