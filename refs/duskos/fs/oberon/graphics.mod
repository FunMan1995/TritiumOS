MODULE Graphics;   (*NW 21.12.89 / 18.11.201 / 8.4.2016*)
  ...
  TYPE
    ...
    Context* = RECORD
        ...
        class: ARRAY 16 OF Oberon.Command
      END;
    ...
    DrawProc = PROCEDURE (obj: Object; VAR msg: Msg);
  ...
  PROCEDURE SetGlobalSel(G: Graph; obj: Object);
  VAR beg, end:INTEGER;
  BEGIN G.sel := obj; SelGraph := G;
    CASE obj OF Caption:
      Texts.Sel := T;
      Texts.SelBegin := obj.pos
      Texts.SelEnd := obj.pos + obj.len;
    END
  END SetGlobalSel;

  PROCEDURE Add*(G: Graph; obj: Object);
  BEGIN obj.marked := FALSE; obj.selected := TRUE; obj.next := G.first;
    G.first := obj; SetGlobalSel(G, obj); G.changed := TRUE
  END Add;
  ...
  BEGIN
    IF obj # NIL THEN obj.selected := TRUE; SetGlobalSel(G, obj) END
  END SelectObj;
  ...
  BEGIN obj := G.first;
    ...
    WHILE obj # NIL DO
      IF (x0 <= obj.x) & (obj.x + obj.w <= x1) & (y0 <= obj.y) & (obj.y + obj.h <= y1) THEN
        obj.selected := TRUE; SetGlobalSel(G, obj)
      END ;
      obj := obj.next
    END ;
  END SelectArea;
  ...
  BEGIN obj := G.first;
    WHILE obj # NIL DO
      ...
      Texts.Write(XW, '/'); GET(ORD(obj)-8, tag); Texts.WriteHex(XW, tag);
      GET(ORD(obj)-4, tag); Texts.WriteHex(XW, tag); Texts.WriteLn(XW); obj := obj.next
    END ;
  ...
  BEGIN F := Files.Ensure(name); Files.Set(W, F, 0); Files.Write(W, GraphFileId);
    InitContext(C); StoreElems(W, C, G.first); Files.Truncate(F); Files.Close(F)
  END WriteFile;
  ...
  PROCEDURE GetClass*(module, allocator: ARRAY OF CHAR; VAR com: Oberon.Command);
  BEGIN com := Oberon.GetCommand(module, allocator);
  ...
  BEGIN obj := NIL; Files.ReadByte(R, cno);
    WHILE ~R.eof & (cno < 255) DO
      ...
      ELSIF C.class[cno] # NIL THEN
        ...
        new.do.read(new, R, C)
      ELSE ReadObj(R, new); Files.ReadByte(R, len); pos := R.pos; Files.Set(R, R.file, pos + len)
      END ;
  ...
  BEGIN G.first := NIL; G.sel := NIL; G.changed := FALSE; F := Files.Old(name);
    IF F # NIL THEN
...
(*PROCEDURE PrintLine(obj: Object; x, y: INTEGER);
  ...
  BEGIN
    CASE M OF
    FontMsg: fnt := M.fnt; w := 0; len := 0; pos := obj(Caption).pos;
...
(*  PROCEDURE PrintMacro(obj: Object; x, y: INTEGER);
  ...
  PROCEDURE InstallDrawMethods*(drawLine, drawCaption, drawMacro: DrawProc);
