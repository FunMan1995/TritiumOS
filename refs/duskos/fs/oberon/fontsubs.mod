MODULE FontSubsetBuilder;
  ...
  BEGIN
    ...
    IF (S.class = Texts.Name) & (b = FontFileId) THEN
      name := S.s;
      FW := Files.New(name); Files.Set(RW, FW, 0); Texts.Scan(S);
      Files.WriteByte(RW, b);
      CopyBytes(RR, RW, 13); (*abstraction, family, variant; height, minX, maxX, minY, maxY *)
      ReadInt16(RR, NofRunsR); NofRunsW := 0; p := RW.pos; WriteInt16(RW, NofRunsW);
      ...
      FOR i := 0 TO NofRunsR - 1 DO
        ...
        WHILE beg < end DO
          WHILE beg > keepto DO
            IF S.class = Texts.Int THEN
              keepfrom := S.n; Texts.Scan(S); ASSERT(keepfrom > keepto + 1);
              IF (S.class = Texts.Char) & ((S.c = '-') OR (S.c = '*')) THEN
                Texts.Scan(S); IF S.class = Texts.Int THEN keepto := S.n; Texts.Scan(S) ELSE keepto := 10000H END
              ELSIF (S.class = Texts.Int) & (S.n < 0) THEN
                keepto := -S.n; Texts.Scan(S)
              ELSE
                keepto := keepfrom
        ...
        END
      END;
      mark := RW.pos; Files.Set(RW, FW, p); WriteInt16(RW, NofRunsW); Files.Set(RW, FW, mark);
